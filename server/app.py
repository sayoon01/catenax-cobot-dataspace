...
- GET  /api/v1/cobot/telemetry/all       → returns all stored records

The server stores each accepted payload as JSON on disk so it can be exposed
by an EDC HttpData asset and replayed into an AAS bridge.

server/app.py
실행 엔트리포인트만 유지 (main, run_server)
"""
...
def read_recent(limit: int) -> List[Dict[str, Any]]:
    ensure_data_dir()
    # include both day-subdirectory files AND flat files in data/
    files = sorted(
